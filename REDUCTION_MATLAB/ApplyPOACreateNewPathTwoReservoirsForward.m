...
function [maxOut,newPath]=ApplyPOACreateNewPathTwoReservoirsForward(completePath,runoffUpArray,runoffDwnArray,numDiv,hBegRangeUp,hEndRangeUp,hBegRangeDwn,hEndRangeDwn,stageSize,turbLimitUp,turbLimitDwn,resCapacityUp,resCapacityDwn)
format shortG;

% completePath is a trajectory already, then columns 2 and 4 is the
% complete path. It doesnt matter if is taken 1,3
pathUp = completePath(:,2); % the path for upstream reservoir
pathDwn = completePath(:,4);% the path for downstream reservoir
len1=size(pathUp);
pathUp(2:len1+1)=pathUp;
pathDwn(2:len1+1)=pathDwn;
pathUp(1)=completePath(1,1);
pathDwn(1)=completePath(1,3);
maxIn=0;
% path format:
% [hUpBeg,hUpEnd,hDwnBeg,hDwnEnd,totalPow,cumulativePow,finalh1Dwn]

newPath=zeros(size(completePath));
finalPath=zeros(size(completePath));
...
for i=1:size(pathUp)-2
    ...
    newPathLinei_1 = [finRes(1),finRes(2),finRes(3),finRes(4),finRes(5)];
    newPathLinei=[finRes(7),finRes(8),finRes(9),finRes(10),finRes(11)];
...
for i=1:length
    ...
    if spilledVolUp>0.01
        spillhUp=v2hUpstream(h2vUpstream(upPath(i+1))-spilledVolUp);
        upPath(i+1)=spillhUp;
        [turbinesOutflowUp,spilledVolUp,powerUp,totaLOutUp,realh1Up]=computePowerUpstream(upPath(i),upPath(i+1),runoffUp(i),inputsUp,stageSize,turbLimitUp,resCapacityUp);
    end
    
    if spilledVolDwn>0.01
        spillhDwn=v2hDownstream(h2vDownstream(dwnPath(i+1))-spilledVolDwn);
        dwnPath(i+1)=spillhDwn;
        [turbinesOutflowDwn,spilledVolDwn,powerDwn,totaLOutDwn,realh1Dwn]=computePowerDownstream(dwnPath(i),dwnPath(i+1),runoffDwn(i),totaLOutUp,stageSize,turbLimitDwn,resCapacityDwn);
    end
    % FORCE TO DO NOT SPILL WATER
     % -----------------------------END
